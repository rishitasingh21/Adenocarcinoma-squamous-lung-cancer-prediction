...
models_dir = r"E:\rishita\code\cnnvall\trainmodel"  # Existing folder for saving models
results_dir = r"E:\rishita\code\cnnvall\train"
...
for fold_no, (train_idx, val_idx) in enumerate(kf.split(dataset), start=1):
    ...
    classification_report_path = os.path.join(results_dir, f"classification_report_fold_{fold_no}.txt")
    with open(classification_report_path, "w") as f:
        f.write(f"Fold {fold_no} Classification Report\n")
        f.write(classification_report(val_labels, val_preds, target_names=folders))
    ...
    if val_acc > best_accuracy:
        ...
        best_fold = fold_no
        torch.save(model.state_dict(), os.path.join(models_dir, f"best_model_fold_{fold_no}.pth"))
